...
const SIDEBAR_STORAGE_KEY: string = "sidebar:open";
...
// Reads the persisted open/closed preference from localStorage. Returns null
// if the user has never explicitly toggled (so callers can fall through to
// `defaultOpen`, typically driven by a viewport-width media query).
//
// We prefer localStorage over the Cookie Store API used upstream because
// (a) this is a client-rendered SPA — no need to read cookies server-side,
// (b) `cookieStore` has uneven Safari support, and (c) synchronous reads
// let `useState`'s lazy initializer avoid a first-paint flash.
...
function writeStoredOpen(open: boolean): void {
  ...
  try {
    ...
    // Ignore quota/privacy-mode failures — the sidebar still works in-session.
  }
}

const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-lg p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding,background-color,color,scale] duration-150 ease-out hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:scale-[0.97] active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
...
export function useSidebarMenuOpen(): SidebarMenuOpenContextValue {
  ...
  if (!ctx) {
    throw new Error("useSidebarMenuOpen must be used within a SidebarProvider.");
  }
  return ctx;
}
...
export function SidebarProvider({
  ...
  // Internal open state. The lazy initializer runs once on mount, so it
  // reads any user preference from localStorage first; if there isn't one,
  // we fall back to `defaultOpen` (typically driven by a viewport media
  // query in the consumer). This keeps "user toggled and refreshed" sticky
  // while still letting the responsive default work on first visit.
...
export function SidebarRail({
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-[left,right,translate,background-color] duration-200 ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:transition-[background-color] after:duration-150 after:ease-out hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
...
export function SidebarGroupLabel({
  ...
  const defaultProps = {
    className: cn(
      "flex h-8 shrink-0 items-center rounded-lg px-2 font-medium text-sidebar-foreground text-xs outline-hidden ring-sidebar-ring transition-[margin,opacity,color] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
export function SidebarMenuAction({
  ...
  const defaultProps = {
    className: cn(
      "absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-hidden ring-sidebar-ring transition-[background-color,color,scale,opacity] duration-150 ease-out hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:scale-[0.96] peer-hover/menu-button:text-sidebar-accent-foreground [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
      // Increases the hit area of the button on mobile.
      "after:absolute after:-inset-2 md:after:hidden",
